...
describe('perf — POST /v1/lessons for write modes (F003)', () => {
  it(`write_pick_word p95 ≤ ${BUDGET_P95_MS} ms`, async () => {
    ...
    console.log(`POST /v1/lessons (write_pick_word): median=${median.toFixed(2)}ms p95=${p95.toFixed(2)}ms (n=${N})`);
    expect(p95).toBeLessThanOrEqual(BUDGET_P95_MS);
  });

  it(`write_type_word p95 ≤ ${BUDGET_P95_MS} ms`, async () => {
    const built = await buildServer({ dbFilename: ':memory:', llm: new RecordedFakeLlm(), repoRoot: REPO_ROOT });
    app = built.app;
    const samples: number[] = [];
    const N = 50;
    ...
    const p95 = percentile(samples, 0.95);
    const median = percentile(samples, 0.5);
    console.log(`POST /v1/lessons (write_type_word): median=${median.toFixed(2)}ms p95=${p95.toFixed(2)}ms (n=${N})`);
    expect(p95).toBeLessThanOrEqual(BUDGET_P95_MS);
  });
});
