...
- **v1.12.0 — Polish + pedagogy round (F011, 4 locked items)** —
  ...
  - **Mariana `.padding(N)` literal sweep**: 9 bare-number `.padding(N)` call-sites across `Sources/Features/` migrated to the `Spacing.*` scale (16→md, 20→lg↑, 24→lg, 28→xl↑, 32→xl). Two round-ups (20→24 in `ExamplesPanelView`, 28→32 in `ListeningLessonView`) documented inline. `DesignTokenContractTests.testNoTokenLiteralsInFeatures` gains a fourth lint regex so future drift fails CI with the same diagnostic shape as the v1.9 / v1.10 lints. Named-edge `.padding(.top, N)` deliberately out of scope this round.
