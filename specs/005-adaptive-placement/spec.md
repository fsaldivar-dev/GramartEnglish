# Feature Specification: Adaptive Placement

**Feature Branch**: `005-adaptive-placement`
**Created**: 2026-05-19 (promoted from `specs/proposals/002-adaptive-placement.md`)
**Status**: Active (selected by PO + TL deliberation after F003 ship)
**Depends on**: F001 (corpus + placement infrastructure)
**Why now**: The actual user reported "me puso en C1 sin saber inglés" — the current placement (24 questions, 4 per CEFR level) over-classifies users as advanced. Even forcing A1 in Settings didn't fully fix the lesson selection. This blocks the core onboarding funnel; every other backlog item assumes users reach lessons at the right level. Fixing placement before shipping more modes restores trust in the level signal.
